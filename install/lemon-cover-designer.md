...
mkdir -p "$SKILLS_DIR"
rm -rf "$SKILLS_DIR/lemon-cover-designer"
cp -r /tmp/lemon-agent-team/skills/lemon-cover-designer "$SKILLS_DIR/"
...
到实际安装位置（方式一由 skills CLI 输出显示，方式二为 `$SKILLS_DIR/lemon-cover-designer/`）确认以下文件全部存在：
