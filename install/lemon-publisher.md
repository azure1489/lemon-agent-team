...
mkdir -p "$SKILLS_DIR"
rm -rf "$SKILLS_DIR/lemon-publisher"
cp -r /tmp/lemon-agent-team/skills/lemon-publisher "$SKILLS_DIR/"
...
到实际安装位置（方式一由 skills CLI 输出显示，方式二为 `$SKILLS_DIR/lemon-publisher/`）确认以下文件全部存在：

- `SKILL.md`
- `references/编辑部通讯录.md`
- `references/游戏资讯日报_交付与流转规范.md`
- `references/html-guide.md`
- `assets/templates/每日花生新闻_样式模版.html`
- `assets/templates/每日花生新闻_索尼发布会_样式模版.html`
