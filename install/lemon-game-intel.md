...
mkdir -p "$SKILLS_DIR"
rm -rf "$SKILLS_DIR/lemon-game-intel"
cp -r /tmp/lemon-agent-team/skills/lemon-game-intel "$SKILLS_DIR/"
...
到实际安装位置（方式一由 skills CLI 输出显示，方式二为 `$SKILLS_DIR/lemon-game-intel/`）确认以下文件全部存在：
